...
@gin.configurable
def align(samples: tf.Tensor, seq_len: int = 160000):
...
def get_dataset(tfrecords_dir):
    ...
    return {
      'train': tf.data.TFRecordDataset(train_filenames, num_parallel_reads=AUTOTUNE)
        .map(parse_tfrecord, num_parallel_calls=AUTOTUNE),
       'validation': tf.data.TFRecordDataset(valid_filenames, num_parallel_reads=AUTOTUNE)
        .map(parse_tfrecord, num_parallel_calls=AUTOTUNE),
       }
